        ...
        for attempt in range(max_retries):
            try:
                ...
                if size_duration_ratio > 15:
                    ...
                    # 发送压缩开始的提示消息
                    progress_message = await update.message.reply_text("正在压缩视频，请稍候...")
                    ...
                    if not os.path.exists(compressed_path) or not os.path.getsize(compressed_path):
                        await progress_message.edit_text("视频压缩失败")
                    ...
                    caption += f"视频压缩完成！已压缩（原始大小/时长比：{size_duration_ratio:.2f}MB/分钟）\n"
